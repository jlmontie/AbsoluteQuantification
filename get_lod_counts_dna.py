...
def get_counts(groupby_obj):
    ...
    for seq_sple in seq_sple_series:
        ...
        for line in file:
            ...
            if cov_info['taxid'] == taxid:
                ...
                total_counts.append(cov_info['read_count'])
    ...
    return {"Organism": groupby_obj[0], "Gene Counts": gene_dict, "Ctrl Counts": ctrl_count_ls, "Total Reads": total_counts}
...
qc_orgs = qc[qc['Sample Name'].str.contains(pattern)]
...
# conc_str = ['d1', 'd2', 'd3', 'd4', 'd5', 'd6']
qc_group = qc_orgs.groupby('Organism')
# count_dict_blast = {}
# for group in qc_group:
#     count_dict_blast.update({f"{orgs_taxid[group[0]]}": get_counts(group, conc_str, 'blast')})
# print(count_dict_blast)
# with open(os.path.join*('data', args.outpath_prefix) + '_blast.json', 'w') as outfile:
#     json.dump(count_dict_blast, outfile)
count_dict_raw = {}
for group in qc_group:
    # print(group)
