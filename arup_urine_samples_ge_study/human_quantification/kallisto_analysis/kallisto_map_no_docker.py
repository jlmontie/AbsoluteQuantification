...
def kallisto_map(file):
    ...
    cmd = f"kallisto quant --single -l 300 -s 30 -i $PWD/{index} -o $PWD/kallisto_output/{outdir} {os.path.join(fastq_dir, file)}"
    # container = client.containers.run('kallisto-img', command=cmd, detach=True)
    # logs = container.logs()
    # for line in container.logs(stream=True):
    #     print (line.strip())
    sp.call(f"{cmd}", shell=True)
...
for file in fastq_file_ls:
    future = pool.submit(kallisto_map, file)
    # kallisto_map(file)
